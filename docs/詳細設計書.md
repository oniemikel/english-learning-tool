...
> 認証方式確定前の旧記述。初期認証をGoogle OAuth専用とする確定後は、`passwordHash`を削除し、`authProvider`および`providerAccountId`を追加する。移行仕様は `docs/認証設計書.md` に従う。
...
- RepositoryでPrismaの主要エラー（重複、外部キー、接続）をアプリケーションエラーへ変換する実装を追加した。
- 認証方式の確定仕様は `docs/認証設計書.md` に定義した。Google OAuth専用、Auth.js JWTセッション、パスワード非保存を初期フェーズの採用方式とする。

| 版数 | 日付    | 内容     | 担当     |
| ---- | ------- | -------- | -------- |
| 2.2 | 2026-07-24 | 設計書の加筆 | 堀内悠斗 |
